...
if ( isset($_POST['cliente']) ){
	...
	mysql_query("UPDATE ordenes SET clienteId='".$clienteId."',fecha='".$fecha."',factura='".$factura."' WHERE idorden='".$id."'");
	...
	mysql_query("DELETE FROM servicios WHERE ordenId='".$id."'");

	for ($i=0; $i < count($servicio); $i++) { 
		$sql[] = "(".$id.",'".$servicio[$i]."','".$descripcion[$i]."','".$preciou[$i]."','".$cantidad[$i]."','".$total[$i]."')";
	}
	
	mysql_query("INSERT INTO servicios(ordenId,servicio,descripcion,unitario,cantidad,precio) VALUES ".implode(",", $sql));
	
	if ( !empty($_POST['anticipo']) ){
		mysql_query("INSERT INTO pagos SET ordenId='".$id."',fecha='".$fecha."',descripcion='Anticipo',cantidad='".$anticipo."',metodopago='".$metodopago."'");
	}

	$errorMsg = '<div class="alert alert-success">
				<i class="fa fa-check"></i> Orden con ID: <strong>'.$id.'</strong> agregada correctamente.
			</div>';

}

$data = mysql_fetch_object(mysql_query("SELECT * FROM ordenes WHERE idorden='".$id."' LIMIT 1"));
?>
	...
	<div class="modal-dialog">
			...
			<div class="modal-body">
				<div class="row">
					<div class="col-sm-12">
						...
						<form role="form" action="admin.php?m=ordenesEditar&id=<?php echo $data->idorden; ?>#pagoAlert" method="post">
		...
		<section class="panel panel-default">
			<header class="panel-heading">
				...
				<i class="fa fa-pencil icon"></i> Editar Orden: <?php echo $data->idorden; ?>
			</header>
	...
	?>
					...
					<div class="form-group text-right">
						<div class="col-lg-12"> 
							...
							<a href="admin.php?m=ordenes" class="btn btn-sm btn-danger"><i class="fa fa-times icon"></i> Cancelar</a>
						</div>
					</div>
				</form>
			</div>
